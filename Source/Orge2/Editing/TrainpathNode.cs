        ...
        public string BrokenStatusString()
        {
            switch (brokenStatus)
            {
                case NodeStatus.SetAsInvalid:
                    return RouteGenExtract.catalog.GetString("Set as invalid in .pat file");
                case NodeStatus.NotOnJunction:
                    return RouteGenExtract.catalog.GetString("Closest junction is too far away");
                case NodeStatus.NotOnTrack:
                    return RouteGenExtract.catalog.GetString("Not able to place node on track");
                case NodeStatus.Dangling:
                    return RouteGenExtract.catalog.GetString("Dangling siding node");
                default:
                    return RouteGenExtract.catalog.GetString("Unknown");
            }
    ...
    public class TrainpathVectorNode : TrainpathNode
    {
        ...
        private float GetSectionStartDistance()
        {
            ...
            for (int tvsi = 0; tvsi < TrackVectorSectionIndex; tvsi++)
            {
                ...
                {
                    distanceFromStart += ORTS.Orge.Drawing.DrawTrackDB.GetLength(trackSection);
                }
            }
