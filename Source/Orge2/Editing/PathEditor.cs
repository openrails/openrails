...
namespace ORTS.Orge.Editing
{
    ...
    public class PathEditor
    {
        ...
        public void EditMetaData(int popupX, int popupY)
        {
            ...
            RouteGenExtract.Localize(metadataDialog);
        ...
        public void ExtendWithPath(ORTS.Menu.Path path)
        {
            ...
            if (newPath.FirstNode == null || newPath.FirstNode.NextMainNode == null) {
                MessageBox.Show(RouteGenExtract.catalog.GetString("The selected path contains no or only 1 node. The current path was not extended."));
                return;
            }
            ...
            {
                action.DoAction();
                MessageBox.Show(RouteGenExtract.catalog.GetString("The selected path has been added as tail and then reconnected."));
            }
            else
            {
                MessageBox.Show(RouteGenExtract.catalog.GetString("The selected path has been added as tail. It was not possible to reconnect automatically."));
            }
        ...
        private bool CanReverse()
        {
            ...
            {
                MessageBox.Show(RouteGenExtract.catalog.GetString("Reversing broken paths is not supported"));
                return false;
            }
            if (CurrentTrainPath.FirstNode == null)
            {
                MessageBox.Show(RouteGenExtract.catalog.GetString("Reversing a path without start node is not supported"));
                return false;
            }
            if (!CurrentTrainPath.HasEnd)
            {
                MessageBox.Show(RouteGenExtract.catalog.GetString("Reversing a path without end node is not supported"));
                return false;
            }
