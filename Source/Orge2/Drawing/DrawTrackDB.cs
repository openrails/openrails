...
namespace ORTS.Orge.Drawing
{
    ...
    public class RouteData
    {
        ...
        public RouteData(string routePath, MessageDelegate messageDelegate)
        {
            ...
            messageDelegate(RouteGenExtract.catalog.GetString("Loading trackfile .trk ..."));
            RouteFile TRK = new RouteFile(MSTS.MSTSPath.GetTRKFileName(routePath));
            RouteName = TRK.Tr_RouteFile.Name;

            messageDelegate(RouteGenExtract.catalog.GetString("Loading track database .tdb ..."));
            TrackDatabaseFile TDB = new TrackDatabaseFile(routePath + @"\" + TRK.Tr_RouteFile.FileName + ".tdb");
            this.TrackDB = TDB.TrackDB;

            messageDelegate(RouteGenExtract.catalog.GetString("Loading tsection.dat ..."));
            ...
            try
            {
                messageDelegate(RouteGenExtract.catalog.GetString("Loading road track database .rdb ..."));
    ...
    public class DrawTrackDB
    {
        ...
        public DrawTrackDB(RouteData routeData, MessageDelegate messageDelegate)
        {
            ...
            messageDelegate(RouteGenExtract.catalog.GetString("Finding the angles to draw signals, endnodes, ..."));
