    ...
    class TerrainTextureManager : Dictionary<string, ReducableTexture2D>, IDisposable
    {
        ...
        public bool TextureIsLoaded(string filename)
        {
            ...
            if (System.IO.File.Exists(path))
            {
                ...
                {
                    messageDelegate(String.Format(RouteGenExtract.catalog.GetString("Loading terrain ace-files {0}-{1} (scaled down with a factor {2})"), loadedAceFilesCounter, loadedAceFilesCounter + 99, CurrentScaleFactor));
                }
        ...
        public void SetCurrentScaleFactor(int newScaleFactor)
        {
            ...
            messageDelegate(String.Format(RouteGenExtract.catalog.GetString("Rescaling previously loaded ace-files")));
