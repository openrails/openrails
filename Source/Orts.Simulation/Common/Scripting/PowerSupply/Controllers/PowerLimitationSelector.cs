...
{
    public abstract class PowerLimitationSelector
    {
        // Internal members and methods (inaccessible from script)
        internal ScriptedPowerLimitationSelector Host;
        internal ScriptedLocomotivePowerSupply PowerSupply => Host.PowerSupply;

        internal void AttachToHost(ScriptedPowerLimitationSelector host)
        {
            Host = host;
        }
        ...
        public List<PowerLimitationSelectorPosition> Positions => Host.Positions;
        ...
        public PantographSelectorPosition PantographSelectorPosition
        {
            get
            {
                switch (PowerSupply)
                {
                    case ScriptedElectricPowerSupply electricPowerSupply:
                        return electricPowerSupply.PantographSelector.Position;
                    case ScriptedDualModePowerSupply dualModePowerSupply:
                        return dualModePowerSupply.PantographSelector.Position;
                    default:
                        return null;
                }
            }
