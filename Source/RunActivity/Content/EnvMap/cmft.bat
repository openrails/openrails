:: - Images downloaded in HDR format from 
...
:: - The resulting dds has some weird color channel order, so need to 
::   use the Nvidia Texture Tools Exporter to 
::     * open the specular_cubemap.dds,
::     * switch off the "Regenerate Mipmaps" option,
::     * save it again to 16x4f RGBA 64 bpp format.
::   The resulting dds will have the same size as the one
::   Generated with CMFT, just with fixed color channel order

:: Download the binary exe from https://github.com/dariomanesku/cmft-bin
...
rem %cmft% %* --input "champagne_castle_1_2k.hdr"           ^
%cmft% %* --input "moonless_golf_2k.hdr"           ^
          ...
          ::Output                          ^
          --outputNum 1                     ^
          --output0 "specular_cubemap"        --output0params dds,rgba16f,cubemap
